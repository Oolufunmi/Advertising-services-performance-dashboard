# Advertisement-services-performance-dashboard
A Tableau-powered dashboard analyzing brand advertising spend, frequency, and cost-efficiency across Cable, Radio, and Terrestrial TV.
### Kindly Note  👇
The links to interact with the dashboards of this Analysis in Tableau are available on this Repository

## Table Of Content
---
- [Objective](#objective)
- [Tools](#tools)
- [Data Cleaning](#data-cleaning)
- [Exploratory Data Analysis](#exploratory-data-analysis)
- [My Data Analysis Approach](#my-data-analysis-approach)
- [Links to Interact with this Dashboards Directly in Tableau](#links-to-interact-with-this-dashboards-directly-in-tableau)
- [Result And Findings ](#result-and-findings)
- [Ideas for Deeper Analysis](#ideas-for-deeper-analysis)
- [Recommendations](#recommendations)
- [Recommdation Summary](#recommendation-summary)
- [References](#references)

### Objective
- To analyze advertising spend, frequency, and efficiency across brands, media types, time slots, and languages using verified media logs for January 2022.

### Tools
- Tableau Public

- Microsoft Excel (for data cleaning and formatting)

### Data Cleaning
- Removed duplicate and irrelevant records

- Standardized brand and media type names

- Converted duration to numeric (in seconds)

- Cleaned inconsistent date and time formats

- Formatted rate and spot count for analysis

### Exploratory Data Analysis
- Checked distributions of spend across brands

- Analyzed cost patterns by date, timebelt, region, and media

- Identified outliers in cost per second

- Examined language and channel breakdowns

### My Data Analysis Approach
- Grouped data by brand, time, region, and language

- Aggregated spend and count metrics

- Calculated cost per second using rate ÷ duration

- Used visual dashboards to highlight patterns and differences

- Compared media types (Cable, Radio, TTV) in terms of both cost and spot count
# Dashboard 1

   <img width="1466" alt="image" src="https://github.com/user-attachments/assets/5f9c39f3-e64f-46f3-8401-5870e13a389e" />
# Dashboard 2

![Uploading image.png…]()

# Dashboard 3
<img width="1466" alt="image" src="https://github.com/user-attachments/assets/b5f07093-68d4-4b1c-b732-65a2d4378c58" />

# Dashboard 4
![Uploading image.png…]()

# Links to Interact with this Dashboards Directly in Tableau


 Check here for Dasboard 1 🫵 - [Here](https://public.tableau.com/views/mediadata_17461865077490/AdvertisingServicesAnalysis1?:language=enUS&publish=yes&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link)
 Check here for Dasboard 2 🫵 - [Here](https://public.tableau.com/views/mediadata_17461865077490/AdvertisingServicesAnalysis2?:language=en-US&publish=yes&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link)
 Check here for Dasboard 3 🫵 - [Here](https://public.tableau.com/views/mediadata_17461865077490/AdvertisingServicesAnalysis3?:language=en-US&publish=yes&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link)
 Check here for Dasboard 4 🫵- [Here](https://public.tableau.com/views/AdvertisingmediaAnaltyics4/AdvertisingServicesAnalysis4?:language=en-US&publish=yes&:sid=&:redirect=auth&:display_count=n&:origin=viz_share_link)

### Result and Findings
- Terra and Maggi had the highest spend overall

- Maggi paid the highest cost per second

- Knorr aired the most spots

- Week 4 and Saturdays had the highest ad spend

- Cable accounted for the majority of total media spend

- English was the dominant language, but Hausa and Pidgin appeared in niche campaigns

- ROS was the top program block by spend

- 30-second ads were most used; 5-second ads had the highest cost per second

### Ideas for Deeper Analysis
- Compare actual airtime vs scheduled spots

- Add audience reach or engagement data if available

- Include product category or campaign type segmentation

- Track ad performance over a longer period (monthly or quarterly trends)

### Recommendations
- Optimize high-cost slots by tracking performance ROI

- Consider shifting some spend from premium spots to cost-effective durations

- Use regional and language targeting more intentionally

- Reduce overdependence on ROS if not delivering value

- Balance spend across weekday and weekend to maximize reach

### Recommendation Summary
- Reduce inefficiencies in placement, explore underutilized regions/languages, and review high-cost program blocks to improve cost per spot and overall media value.

### References
- Data Source - Internal media log (January 2022)

- Tableau Public visual dashboards

- Raw CSV data from verified ad tracking source
|Heading1|Heading2|
|-------|----------|
|content1|content2|
|Python1|pyhton2|
|MySql1|Mysql2|


#another way to write code here 🫵

*to write in italic*


😄
💻 
🙂
🆒 😄 💻 🙂 🆒 😄 💻 🙂 🆒 😄 💻 🙂 🆒 😄 💻 🙂 🆒 😄 💻 🙂 🆒 😄 💻 🙂 🆒 











